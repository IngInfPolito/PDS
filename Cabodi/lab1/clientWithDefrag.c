...
int main (int argc, char* argv[]) {
	...
	while ((nr = leggiPagina(heap, page, fpI, N)) > 0) {
		...
		// Defrag and update pointers
		heapDefrag(heap);
		save = heapNewPointer(heap, save);
		page = heapNewPointer(heap, page);
		if (save->array != NULL) {
			save->array = heapNewPointer(heap, save->array);
			for (i = 0; i < save->num; i++) {
				save->array[i] = heapNewPointer(heap, save->array[i]);
			}
		}
		heapDefragClose(heap);
	} 
